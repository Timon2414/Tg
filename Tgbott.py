...
index ae1693b7d5fdc3a3e98f199d91142a05bb1ce613..9f0c86c7e28f18b025101fbb5031158d690f77e3 100644
--- a/Tgbot.py
+++ b/Tgbot.py
@@ -1,79 +1,81 @@
 import asyncio
...
@@ -84,1403 +86,1999 @@ order_id_counter = itertools.count(1)
...
+user_last_success: Dict[int, datetime] = {}
+user_usernames: Dict[int, Optional[str]] = {}
+
+# Контакт с админом
+user_contact_target: Dict[int, int] = {}
+user_contact_selecting: Set[int] = set()
+contact_admin_messages: Dict[Tuple[int, int], int] = {}
+user_waiting_admin_reply: Dict[int, int] = {}
+
+# Управление ценами/сервисами/админами
+admin_waiting_remove_service: Dict[int, str] = {}
+admin_waiting_add_admin: Set[int] = set()
+admin_waiting_remove_admin: Set[int] = set()
+admin_broadcast_wait: Set[int] = set()
+
+
+# Статистика по неделям
+def reset_weekly_stats(start: date):
+    return {
+        "week_start": start,
+        "days": {i: {"total": 0, "amount": 0.0, "users": {}} for i in range(7)},
+    }
+
+
+def get_week_start_for(dt: datetime) -> date:
+    current_date = dt.date()
+    return current_date - timedelta(days=current_date.weekday())
+
+
+weekly_stats = reset_weekly_stats(get_week_start_for(datetime.now()))
+
...
+def get_number_input_kb() -> InlineKeyboardMarkup:
...
+def get_contact_admins_kb() -> InlineKeyboardMarkup:
+    rows = []
+    for admin_id in sorted(ADMINS):
+        label = (
+            ADMIN_LABELS.get(admin_id)
+            or (user_usernames.get(admin_id) and f"@{user_usernames[admin_id]}")
+            or f"Админ {admin_id}"
+        )
+        rows.append(
+            [
+                InlineKeyboardButton(
+                    text=label, callback_data=f"contact_select:{admin_id}"
+                )
+            ]
+        )
+    rows.append([InlineKeyboardButton(text="⬅️ В меню", callback_data="user:back_main")])
+    return InlineKeyboardMarkup(inline_keyboard=rows)
+
+
 def get_admin_panel_kb() -> InlineKeyboardMarkup:
     # здесь глобал не нужен, мы только читаем переменную
-    break_text = "⏸ Перерыв (все сервисы)" if not break_mode else "▶️ Выход из перерыва (все)"
+    break_text = (
+        "⏸ Перерыв (все сервисы)" if not break_mode else "▶️ Выход из перерыва (все)"
+    )
     return InlineKeyboardMarkup(
         inline_keyboard=[
...
+            [
+                InlineKeyboardButton(
+                    text="⏸ Перерывы по сервисам", callback_data="admin:svc_breaks"
+                )
+            ],
+            [
+                InlineKeyboardButton(
+                    text="💵 Изменить цены", callback_data="admin:prices"
+                )
+            ],
+            [
+                InlineKeyboardButton(
+                    text="➕ Добавить сервис", callback_data="admin:add_service"
+                )
+            ],
+            [
+                InlineKeyboardButton(
+                    text="🗑 Удалить сервис", callback_data="admin:remove_service"
+                )
+            ],
+            [
+                InlineKeyboardButton(
...
+            [
+                InlineKeyboardButton(
+                    text="✅ Взять", callback_data=f"order_take:{order_id}"
+                )
+            ],
+            [
+                InlineKeyboardButton(
+                    text="🚫 Не брать", callback_data=f"order_nottake:{order_id}"
+                )
+            ],
         ]
     )
...
+                InlineKeyboardButton(
+                    text="📨 Запрошен код", callback_data=f"status:{order_id}:code"
+                )
+            ],
+            [
+                InlineKeyboardButton(
+                    text="✅ Встал", callback_data=f"status:{order_id}:ok"
+                ),
+                InlineKeyboardButton(
+                    text="❌ Не встал", callback_data=f"status:{order_id}:bad"
+                ),
+            ],
+            [
+                InlineKeyboardButton(
+                    text="🚫 Отменён", callback_data=f"status:{order_id}:cancel"
+                )
+            ],
+            [
+                InlineKeyboardButton(
+                    text="🔁 Переназначить", callback_data=f"order_reassign:{order_id}"
+                )
             ],
...
+                InlineKeyboardButton(
+                    text="❌ Отклонить выплату", callback_data=f"wd_status:{wid}:rej"
+                ),
+                InlineKeyboardButton(
+                    text="✅ Выплачено", callback_data=f"wd_status:{wid}:paid"
+                ),
             ]
         ]
     )
...
+def get_order_admin_display(order: Order) -> str:
+    if order.assigned_admin_username:
+        return f"@{order.assigned_admin_username}"
+    if order.assigned_admin_id:
+        return get_admin_label(order.assigned_admin_id)
+    return "не назначен"
+
+
+def register_user(uid: int, username: Optional[str] = None):
+    all_users.add(uid)
+    if username:
+        user_usernames[uid] = username
+
+
+def get_service_title(service_key: str, fallback: str = "Сервис") -> str:
+    data = services.get(service_key)
+    if data:
+        return data.get("title", fallback)
+    return fallback or service_key
+
+
+def get_order_service_name(order: Order) -> str:
+    return get_service_title(
+        order.service_key, order.service_title or order.service_key
+    )
+
+
+def ensure_week_is_fresh(now: datetime):
+    global weekly_stats
+    current_start = get_week_start_for(now)
+    if weekly_stats.get("week_start") != current_start:
+        weekly_stats = reset_weekly_stats(current_start)
+
+
+def update_weekly_stats(order: Order):
+    ensure_week_is_fresh(datetime.now())
+    weekday = datetime.now().weekday()
+    day_stat = weekly_stats["days"][weekday]
+    day_stat["total"] += 1
+    day_stat["amount"] += order.price
+    user_data = day_stat["users"].setdefault(
+        order.user_id, {"username": order.username, "count": 0, "amount": 0.0}
+    )
+    user_data["username"] = order.username
+    user_data["count"] += 1
+    user_data["amount"] += order.price
+
+
 # --------------------------- ИНИЦИАЛИЗАЦИЯ БОТА ---------------------------
...
+    service_name = get_order_service_name(order)
...
+
 ...
 async def cmd_start(message: Message):
     ...
     uid = user.id
+    register_user(uid, user.username)
...
+    # Ответ администратора пользователю (контакт)
+    if is_admin(uid) and message.reply_to_message:
+        key = (uid, message.reply_to_message.message_id)
+        if key in contact_admin_messages:
+            target_user = contact_admin_messages.pop(key)
+            user_waiting_admin_reply.pop(target_user, None)
+            admin_label = ADMIN_LABELS.get(
+                uid, message.from_user.username or f"Admin {uid}"
+            )
+            try:
+                await bot.send_message(
+                    target_user,
+                    f"✉️ Ответ от администратора <b>{admin_label}</b>:\n\n{message.text or ''}",
+                )
+            except Exception:
+                pass
+            await message.answer("Ответ пользователю отправлен.")
+            return
+
...
+            and message.reply_to_message.message_id == cr.request_message_id
+        ):
             # Проверяем срок
...
+                await message.answer(
+                    "⏰ Время на ввод кода уже истекло, заявка отменена."
+                )
                 return
...
+    # Сообщение пользователю для админа
+    if not text.startswith("/") and uid in user_contact_target:
+        admin_id = user_contact_target.pop(uid)
+        if admin_id not in ADMINS:
+            await message.answer(
+                "Администратор больше недоступен. Попробуйте выбрать другого."
+            )
+            return
+        user_waiting_admin_reply[uid] = admin_id
+        admin_label = ADMIN_LABELS.get(admin_id, f"Админ {admin_id}")
+        user_mention = get_user_mention_by(uid, message.from_user.username)
+        try:
+            msg_to_admin = await bot.send_message(
+                admin_id,
+                "📨 Новый вопрос от пользователя\n\n"
+                f"От: {user_mention} (ID: <code>{uid}</code>)\n"
+                f"Вопрос: {message.text}\n\n"
+                "Ответьте <b>на это сообщение</b>, чтобы пользователь получил ваш ответ.",
+            )
+            contact_admin_messages[(admin_id, msg_to_admin.message_id)] = uid
+        except Exception:
+            await message.answer(
...
+
+        await message.answer(
+            "Ваше сообщение отправлено администратору. Ожидайте ответ.",
...
+        await message.answer(
+            f"💵 Цена для сервиса <b>{service_name}</b> обновлена: <b>{value}$</b>."
+        )
+        return
+
+    # Добавление администратора
+    if not text.startswith("/") and uid in admin_waiting_add_admin:
+        if not is_admin(uid):
+            admin_waiting_add_admin.discard(uid)
+            await message.answer("⛔ Вы не администратор.")
+            return
...
+        ADMIN_LABELS.setdefault(new_admin_id, f"Админ {new_admin_id}")
+        admin_waiting_add_admin.discard(uid)
+        await message.answer(
...
+    if not text.startswith("/") and uid in admin_waiting_remove_admin:
+        if not is_admin(uid):
+            admin_waiting_remove_admin.discard(uid)
+            await message.answer("⛔ Вы не администратор.")
+            return
+        if text.lower() == "отмена":
+            admin_waiting_remove_admin.discard(uid)
...
+            return
+        if remove_id == uid:
+            await message.answer("⚠️ Нельзя удалить самого себя.")
+            return
...
+        admin_waiting_remove_admin.discard(uid)
+        await message.answer(f"Администратор с ID <code>{remove_id}</code> удалён.")
+        return
+
+    # Рассылка
+    if not text.startswith("/") and uid in admin_broadcast_wait:
+        if not is_admin(uid):
...
+        admin_broadcast_wait.discard(uid)
+        await message.answer(
+            f"Рассылка завершена. Успешно: <b>{sent}</b>, ошибок: <b>{failed}</b>."
+        )
         return
...
+                await message.answer(
+                    "❗ Название сервиса не может быть пустым. Введите ещё раз:"
+                )
                 return
...
+            base = (
+                "".join(ch.lower() for ch in name if ch.isalnum() or ch == "_") or "srv"
+            )
             key = base
             i = 1
...
+                "Например: <code>+79991234567</code>",
+                reply_markup=get_number_input_kb(),
             )
...
+        # Проверка на дубликат по сервису
+        for oid in user_orders.get(uid, []):
+            o = orders.get(oid)
+            if not o:
+                continue
+            if o.phone == phone and o.service_key == service_key:
+                await message.answer(
+                    "⚠️ Вы уже сдавали этот номер в выбранный сервис. Повторная сдача невозможна."
+                )
+                return
+
         # Создаём заявку
...
+                    admin_id, text_admin, reply_markup=get_admin_take_kb(new_id)
                 )
...
+            await message.answer(
+                "📋 У вас нет номеров, которые ещё не были взяты админами."
+            )
             return
...
+            service_name = get_service_title(
+                o.service_key, o.service_title or o.service_key
             )
+            lines.append(f"#{o.id} | {dt} | {service_name} | <code>{o.phone}</code>")
...
+            f"💰 Ваш баланс: <b>{balance:.2f}$</b>", reply_markup=get_balance_kb()
         )
         return
...
+        if uid in user_waiting_admin_reply:
+            await message.answer(
+                "⌛ Ожидайте ответ на предыдущий вопрос от администратора."
+            )
+            return
+        user_contact_selecting.add(uid)
         await message.answer(
-            "📨 Для связи с администратором напишите сюда свой вопрос.\n\n"
-            "Админ увидит ваш username / ID и сможет с вами связаться."
+            "Выберите администратора, которому хотите задать вопрос:",
+            reply_markup=get_contact_admins_kb(),
         )
         return
...
+
+
+@dp.callback_query(F.data.startswith("contact_select:"))
+async def contact_select(call: CallbackQuery):
+    uid = call.from_user.id
+    register_user(uid, call.from_user.username)
+    user_contact_selecting.discard(uid)
+    try:
+        _, admin_str = call.data.split(":", 1)
...
+
+    if admin_id not in ADMINS:
+        await call.answer("Администратор недоступен.", show_alert=True)
+        return
+
+    user_contact_target[uid] = admin_id
+    label = ADMIN_LABELS.get(admin_id, f"Админ {admin_id}")
+    await call.message.edit_text(
+        f"Вы выбрали администратора: <b>{label}</b>.\n"
+        "Отправьте свой вопрос одним сообщением.",
...
+async def cancel_contact(call: CallbackQuery):
+    uid = call.from_user.id
+    register_user(uid, call.from_user.username)
+    user_contact_target.pop(uid, None)
+    user_contact_selecting.discard(uid)
+    await call.message.edit_text("Диалог с администратором отменён.")
+    await call.answer()
+
+
...
+            1 for o in orders.values() if o.user_id == uid and o.status == "accepted"
         )
...
+                    admin_id, text_admin, reply_markup=get_withdraw_admin_kb(w.id)
                 )
                 msgs.append((admin_id, msg.message_id))
             except Exception:
                 pass
...
+            uid, "Вы в главном меню.", reply_markup=get_main_keyboard(is_admin_flag)
         )
         await call.answer()
...
+            f"Неделя: <b>{week_start.strftime('%d.%m')} - {week_end.strftime('%d.%m')}</b>",
+            "\nВыберите день, чтобы посмотреть детали по пользователям.",
+        ]
+        kb_rows = []
+        for i in range(7):
+            day_stat = weekly_stats["days"].get(i, {"total": 0, "amount": 0.0})
+            lines.append(
+                f"{weekday_names[i]}: успешных номеров <b>{day_stat.get('total', 0)}</b>, сумма <b>{day_stat.get('amount', 0.0):.2f}$</b>"
+            )
+            kb_rows.append(
+                [
+                    InlineKeyboardButton(
+                        text=weekday_names[i], callback_data=f"stats_day:{i}"
+                    )
+                ]
+            )
+        kb_rows.append(
...
+            label = ADMIN_LABELS.get(aid, f"Админ {aid}")
+            lines.append(f"• {label} (ID: {aid})")
+        kb = InlineKeyboardMarkup(
+            inline_keyboard=[
+                [
+                    InlineKeyboardButton(
+                        text="➕ Добавить", callback_data="admin:add_admin"
+                    )
+                ],
+                [
+                    InlineKeyboardButton(
+                        text="➖ Удалить", callback_data="admin:remove_admin"
+                    )
+                ],
+                [InlineKeyboardButton(text="⬅️ Назад", callback_data="admin:back")],
+            ]
...
+
+    if action == "add_admin":
+        admin_waiting_add_admin.add(uid)
+        await call.message.edit_text(
+            "Введите ID пользователя, которого нужно сделать администратором (только число).\nДля отмены отправьте слово <b>отмена</b>."
+        )
+        await call.answer()
+        return
+
+    if action == "remove_admin":
+        admin_waiting_remove_admin.add(uid)
+        await call.message.edit_text(
...
+
+    if action == "broadcast":
+        admin_broadcast_wait.add(uid)
+        await call.message.edit_text(
+            "Отправьте сообщение для рассылки всем пользователям.\nНапишите <b>отмена</b> для отмены."
+        )
+        await call.answer()
+        return
+
...
+        if not all_users:
+            await call.message.edit_text(
+                "Пользователей пока нет.", reply_markup=get_admin_panel_kb()
+            )
             await call.answer()
             return
-
...
+        for uid_value in sorted(all_users):
+            last = user_last_success.get(uid_value)
+            last_text = last.strftime("%d.%m %H:%M") if last else "не сдавал"
             lines.append(
-                f"#{o.id} | {dt} | {service_name} | <code>{o.phone}</code> | "
-                f"пользователь: {mention} | цена: <b>{o.price}$</b>"
+                f"• {get_user_mention_by(uid_value, None)} | последняя сдача: <b>{last_text}</b>"
             )
-
...
-        ]
+        active_orders = [o for o in orders.values() if o.status in ("new", "taken")]
         if not active_orders:
-            await call.message.edit_text("📞 Активных номеров нет.", reply_markup=get_admin_panel_kb())
+            await call.message.edit_text(
+                "📞 Активных номеров нет.", reply_markup=get_admin_panel_kb()
+            )
             await call.answer()
             return
...
+                o.service_key, o.service_title or o.service_key
+            )
+            admin_display = get_order_admin_display(o)
             lines.append(
-                f"#{o.id} | {dt} | {service_name} | <code>{o.phone}</code> | пользователь: {get_user_mention(o)} | статус: <b>{format_status(o.status)}</b>"
+                f"#{o.id} | {dt} | {service_name} | <code>{o.phone}</code> | пользователь: {get_user_mention(o)} | админ: <b>{admin_display}</b> | статус: <b>{format_status(o.status)}</b>"
             )
             kb_rows.append(
-                [InlineKeyboardButton(
-                    text=f"Открыть #{o.id}",
...
+            await call.message.edit_text(
+                "💸 Заявок на вывод пока нет.", reply_markup=get_admin_panel_kb()
+            )
             await call.answer()
             return
...
+                [
+                    InlineKeyboardButton(
+                        text=f"Открыть вывод #{w.id}", callback_data=f"wd_open:{w.id}"
+                    )
+                ]
             )
         kb_rows.append(
...
+        if not services:
+            await call.message.edit_text(
+                "Нет доступных сервисов.", reply_markup=get_admin_panel_kb()
+            )
+            await call.answer()
+            return
+
+        lines = ["🗑 <b>Удаление сервиса</b>:\n"]
+        kb_rows = []
+        for key, data in services.items():
...
+                [
+                    InlineKeyboardButton(
+                        text=f"Удалить {data['title']}", callback_data=f"svc_del:{key}"
+                    )
+                ]
+            )
+        kb_rows.append(
...
+        await call.message.edit_text(
+            "🛠 Админ-панель", reply_markup=get_admin_panel_kb()
+        )
         await call.answer()
         return
...
+            [
+                InlineKeyboardButton(
+                    text=f"{data['title']} ({state})", callback_data=f"svc_break:{key}"
+                )
+            ]
         )
-    kb_rows.append(
-        [InlineKeyboardButton(text="⬅️ Назад", callback_data="admin:back")]
-    )
+    kb_rows.append([InlineKeyboardButton(text="⬅️ Назад", callback_data="admin:back")])
     kb = InlineKeyboardMarkup(inline_keyboard=kb_rows)
     await call.message.edit_text("\n".join(lines), reply_markup=kb)
...
+async def stats_day_details(call: CallbackQuery):
+    uid = call.from_user.id
+    register_user(uid, call.from_user.username)
+    if not is_admin(uid):
+        await call.answer("Нет доступа.", show_alert=True)
...
+        "Воскресенье",
+    ]
+    day_stat = weekly_stats["days"].get(
+        day_index, {"total": 0, "amount": 0.0, "users": {}}
+    )
+    lines = [
+        f"📊 <b>{weekday_names[day_index]}</b>",
+        f"Всего успешных: <b>{day_stat.get('total', 0)}</b>",
+        f"Сумма: <b>{day_stat.get('amount', 0.0):.2f}$</b>",
+        "\nПо пользователям:",
+    ]
+    if not day_stat.get("users"):
+        lines.append("Нет данных за этот день.")
+    else:
+        for user_id, info in day_stat["users"].items():
+            mention = get_user_mention_by(user_id, info.get("username"))
+            balance = user_balances.get(user_id, 0.0)
+            lines.append(
+                f"• {mention}: <b>{info.get('count', 0)}</b> успешных, сумма <b>{info.get('amount', 0.0):.2f}$</b>, баланс <b>{balance:.2f}$</b>"
+            )
+
+    kb = InlineKeyboardMarkup(
+        inline_keyboard=[
...
+async def svc_delete(call: CallbackQuery):
+    uid = call.from_user.id
+    register_user(uid, call.from_user.username)
+    if not is_admin(uid):
+        await call.answer("Нет доступа.", show_alert=True)
...
+        return
+
+    title = services[service_key]["title"]
+    services.pop(service_key, None)
+    service_breaks.pop(service_key, None)
+
+    await call.message.edit_text(
+        f"Сервис <b>{title}</b> удалён.", reply_markup=get_admin_panel_kb()
+    )
+    await call.answer("Сервис удалён.")
+
+
...
-    service_name = services[order.service_key]["title"]
+    ADMIN_LABELS.setdefault(uid, call.from_user.username or f"Админ {uid}")
+
+    service_name = get_order_service_name(order)
...
+        await call.answer(
+            "Вы не являетесь ответственным за эту заявку.", show_alert=True
+        )
         return
...
+            [
+                InlineKeyboardButton(
+                    text=label, callback_data=f"order_reassign_to:{order_id}:{admin_id}"
+                )
+            ]
         )
     kb_rows.append(
...
+        [
+            InlineKeyboardButton(
+                text="⬅️ Назад", callback_data=f"admin_open_order:{order_id}"
+            )
+        ]
     )
     kb = InlineKeyboardMarkup(inline_keyboard=kb_rows)
...
+            reply_markup=get_admin_status_kb(order.id),
+        )
+        order_admin_messages.setdefault(order_id, []).append(
+            (new_admin_id, msg.message_id)
         )
...
+    if order.assigned_admin_id:
+        text += f"\nОтветственный: <b>{get_order_admin_display(order)}</b>"
+
     kb = None
...
+        user_last_success[user_id] = datetime.now()
+        update_weekly_stats(order)
...
+                f"Сумма: <b>{w.amount:.2f}$</b>",
             ...
             pass
+        # Убираем из истории пользователя
+        user_withdrawals[w.user_id] = [
+            wid for wid in user_withdrawals.get(w.user_id, []) if wid != w.id
+        ]
